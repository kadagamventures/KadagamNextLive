...
console.log("⏳ ACCESS TOKEN EXPIRES =", process.env.JWT_ACCESS_EXPIRES);
console.log("🔄 REFRESH TOKEN EXPIRES =", process.env.JWT_REFRESH_EXPIRES);
...
const tokenUtils = {
  generateToken: (payload, expiresIn) => {
    if (!process.env.JWT_SECRET) {
      throw new Error("❌ Cannot sign token: JWT_SECRET is undefined");
    }
  ...
  logoutUser: async (accessToken, refreshToken) => {
    try {
      ...
      if (refreshToken) {
        ...
        if (refreshDecoded) {
          await tokenUtils.blacklistToken(refreshToken, 7 * 24 * 60 * 60); // 7 days
        }
      }
