...
const startWork = asyncHandler(async (req, res, next) => {
  ...
  try {
    // service now returns { attendance, scheduledEndTime }
    ...
    res.status(201).json({
      ...
      data: {
        ...
        scheduledEndTime // frontend can schedule auto-checkout
      },
    });
  } catch (err) {
...
/**
 * 🚀 Fetch Ongoing Attendance Session (persist across reload)
 */
...
const getActiveAttendance = asyncHandler(async (req, res) => {
  ...
  const elapsed = Math.floor((Date.now() - new Date(session.checkInTime)) / 1000);
  res.status(200).json({
    success:     true,
    isWorking:   true,
    attendanceId: session._id,
