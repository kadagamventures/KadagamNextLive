...
const StaffForm = () => {
  ...
  useEffect(() => {
    if (!projectsList.length) dispatch(fetchProjects());
  }, [projectsList.length, dispatch]);

  useEffect(() => {
    if (isEditMode) dispatch(fetchStaffById(id));
  ...
  const validateStaffId = useCallback(
    debounce(async (staffId) => {
      ...
      try {
        ...
        setSuggestedStaffId(err.response?.data?.suggestedId || "");
      }
    }, 500),
    []
  );
  ...
  const handleProjectChange = (selectedOptions) => {
    ...
    setNewStaff((prev) => ({ ...prev, assignedProjects: ids }));
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
  <label className="block text-sm text-gray-500 mb-1">Profile Picture (Optional)</label>
  <input
    key={profilePicKey}
    type="file"
    name="profilePic"
    ...
    <div className="mt-2 flex items-center gap-2">
      ...
      <button
        onClick={() =>
          openImageModal(
            newStaff.profilePic instanceof File
              ? newStaff.profilePic.preview || URL.createObjectURL(newStaff.profilePic)
              : newStaff.profilePic
          )
        }
      ...
      >
        <FaTrashAlt className="w-4 h-4 text-red-600" />
              ...
              {/* Resume Upload */}
              <div>
  <label className="block text-sm text-gray-500 mb-1">Resume (Optional)</label>
  <input
    key={resumeKey}
    type="file"
    name="resume"
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2">
                {permissionOptions.map((perm) => {
                  ...
                  if (perm === "manage_staff") displayLabel = "Add Staff";
                  return (
                    <label
                      key={perm}
